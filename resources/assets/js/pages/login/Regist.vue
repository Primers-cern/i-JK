<template>
  <div>
    ...
    <div class="inputItem">
      ...
      <button class="btn getVeri" @click="handleGetVerification" ref="codeBtn">{{getVeri}}</button>
    </div>
    <div class="inputItem">
...
export default {
      ...
      if (this.getCode) {
        ...
        let timeout = setTimeout(() => {
          if (this.code == this.getCode) {
            this.$refs.code.style.backgroundColor = '#ceffd6'
            this.$refs.phone.disabled = true //阻止修改手机
            this.OKCode = true
          } else {
            this.$refs.code.style.backgroundColor = '#ffcece'
            this.OKCode = false
          }
        },500)
      }
  ...
  mounted () {
    // 获取用户协议
    axios.get('/MerchantUser/agreement')
      .then((res) => {
